...
// Using the provided image as the hero background
const heroBackgroundImage = "https://images.unsplash.com/photo-1545579133-99bb5ab189bd?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTB8fHRyb3BpY2FsJTIwaXNsYW5kfGVufDB8fDB8fHww&fm=jpg&q=60&w=3000";

// Razorpay Key - Replace with your actual test/live key
...
const ScheduleCard = ({ schedule, passengers, travelDate, onSelectClass }) => {
  ...
  const journeyDuration = schedule.estimated_duration_minutes 
    ? `${Math.floor(schedule.estimated_duration_minutes / 60)}h ${schedule.estimated_duration_minutes % 60}m`
      ...
      <div className="p-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-800">{schedule.operator_name}</h3>
                <p className="text-sm text-gray-500">{schedule.ferry_name}</p>
              </div>
            </div>
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold text-blue-600">
              ₹{Math.min(...schedule.available_classes.map((c) => parseFloat(c.price_per_seat)))}
            </p>
            <p className="text-xs text-gray-500 mt-1">{passengers} passenger(s)</p>
            <Button
              onClick={() => setExpanded(!expanded)}
              className="mt-3 bg-blue-600 hover:bg-blue-700 text-white"
            >
              {expanded ? 'Close' : 'View Classes'}
            </Button>
          </div>
        </div>
        ...
        {expanded && (
          <div className="mt-6 border-t pt-6">
            <h4 className="font-bold text-lg mb-4">Select Seat Class</h4>
...
// Seat Class Option Component
const SeatClassOption = ({ seatClass, schedule, passengers, travelDate, onSelect }) => {
  const pricePerSeat = parseFloat(seatClass.price_per_seat);
  ...
  return (
    <div className="border rounded-lg p-4 hover:border-blue-500 transition-colors">
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <h5 className="font-bold text-lg">{seatClass.class_name}</h5>
          ...
          <p className="text-sm text-gray-600 mt-3 flex items-center gap-1">
            ...
            Available Seats: <span className="font-semibold">{seatClass.available_seats}</span>
          </p>
        </div>
        <div className="text-right ml-4">
          <p className="text-sm text-gray-500">₹{pricePerSeat} per seat</p>
          <p className="text-2xl font-bold text-gray-800">₹{totalPrice}</p>
          <p className="text-xs text-gray-500">Total for {passengers} pax</p>
          <Button
            onClick={() => onSelect(schedule.schedule_id, seatClass, totalPrice, schedule)}
            className="mt-3 bg-green-600 hover:bg-green-700 text-white"
            disabled={seatClass.available_seats < passengers}
          >
            {seatClass.available_seats < passengers ? 'Not Available' : 'Book Now'}
          </Button>
        </div>
      </div>
    </div>
        ...
        <div className="p-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              {schedule && (
                <>
                  <p><span className="font-semibold">Ferry:</span> {schedule.ferry_name}</p>
                  <p><span className="font-semibold">Operator:</span> {schedule.operator_name}</p>
                  <p><span className="font-semibold">Route:</span> {schedule.departure_port} → {schedule.arrival_port}</p>
                  <p><span className="font-semibold">Departure:</span> {schedule.departure_time.slice(0, 5)}</p>
                </>
              )}
              <p><span className="font-semibold">Date:</span> {travelDate}</p>
              <p><span className="font-semibold">Class:</span> {seatClass?.class_name}</p>
            ...
            {passengerDetails.map((passenger, index) => (
              <div key={index} className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>Gender *</Label>
                    <Select value={passenger.gender} onValueChange={(value) => handlePassengerChange(index, 'gender', value)}>
                  ...
                  <div>
                    ...
                    <Select value={passenger.id_proof_type} onValueChange={(value) => handlePassengerChange(index, 'id_proof_type', value)}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
          ...
          <div className="mt-6 flex gap-4">
            ...
            <Button onClick={handlePayment} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white" disabled={loading}>
              {loading ? <LoadingSpinner size="sm" /> : <><CreditCard className="h-4 w-4 mr-2" />Proceed to Payment</>}
            </Button>
          </div>
        </div>
      </div>
...
const Home = () => {
  ...
  const fetchOperators = async () => {
    try {
      const response = await api.get('/ferry/operators');
      if (response.data.success) {
        setOperators(response.data.data);
      }
    } catch (error) {
      console.error('Error fetching operators:', error);
      // Not showing an alert for this as it's a background task
    }
  };

  const handleSearch = async () => {
    if (!searchParams.from || !searchParams.to) {
      setAlert({ type: 'error', message: 'Please select departure and arrival ports' });
      return;
    }
    if (searchParams.from === searchParams.to) {
      setAlert({ type: 'error', message: 'Departure and arrival ports cannot be the same.' });
      return;
    }
    if (!searchParams.date) {
      setAlert({ type: 'error', message: 'Please select a travel date' });
      return;
    }
    if (searchParams.adults < 1) {
      setAlert({ type: 'error', message: 'At least one adult passenger is required' });
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.data.length === 0) {
          setAlert({ type: 'info', message: 'No ferries found for your search criteria. Try different dates or routes.' });
        }
      } else {
        setAlert({ type: 'error', message: response.data.msg || 'Failed to fetch schedules' });
      }
    } catch (error) {
      console.error('Search error:', error);
      setAlert({ type: 'error', message: error.response?.data?.msg || 'An error occurred. Please try again.' });
      setSearchResults([]);
      setShowResults(true); // Show the "no results" message
    } finally {
      setLoading(false);
    }
  };

  const handleSelectClass = (scheduleId, seatClass, totalAmount, schedule) => {
    if (!user) {
      setAlert({ type: 'warning', message: 'Please log in to book your voyage.' });
  ...
  const closeBookingModal = () => {
    setBookingModal({ isOpen: false, scheduleId: null, seatClass: null, passengers: 0, travelDate: '', totalAmount: 0, schedule: null });
  };
    ...
    <>
      <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{ backgroundImage: `url(${heroBackgroundImage})` , filter: 'brightness(1)' }}
        ></div>
        {/* Overlay */}
        <div className="absolute inset-0 bg-black/30"></div>
        
        {/* Main Content */}
        <div className="relative z-10 w-full min-h-screen flex flex-col items-center justify-between p-4 sm:p-8 text-white">
          {/* Decorative Frame */}
          ...
          <div  className="flex-grow flex flex-col items-center justify-center text-center">
            <h1 style={customFontStyle} className="text-5xl sm:text-6xl lg:text-8xl font-extrabold tracking-tight drop-shadow-lg">
              Find Your Perfect <span style={customFontStyle2} className="text-yellow-400">Voyage</span>
            </h1>
            <p style={customFontStyle} className="mt-4 max-w-3xl text-lg sm:text-xl text-white/90 drop-shadow-md">
              Discover Premier Ferry Services In Breathtaking Locations. Unplug, Unwind, And Reconnect With The Sea.
            </p>
          </div>
          
          {/* New Search Bar */}
          <div className="w-full max-w-5xl mb-36 bg-white/90 backdrop-blur-md rounded-full shadow-2xl p-2 sm:p-3 mt-auto">
             <div className="flex flex-col sm:flex-row items-center divide-y sm:divide-y-0 sm:divide-x divide-gray-200 text-gray-700">
                {/* From Port */}
                <div className="flex items-center gap-2 px-4 py-2 sm:py-0 w-full sm:w-auto flex-1">
                        ...
                        <span className="text-xs font-semibold text-gray-500">From</span>
                         <Select value={searchParams.from} onValueChange={(value) => setSearchParams({ ...searchParams, from: value })}>
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="Havelock Island">Havelock Island</SelectItem>
                ...
                {/* Date */}
                 <div className="flex items-center gap-2 px-4 py-2 sm:py-0 w-full sm:w-auto flex-1">
                    ...
                    <div className="flex flex-col items-start w-full">
                        ...
                        <Input id="date" type="date" min={today} value={searchParams.date}
                ...
                <div className="flex items-center gap-2 px-4 py-2 sm:py-0 w-full sm:w-auto flex-1">
                    ...
                    <div className="flex flex-col items-start w-full">
                        ...
                        <Input type="number" min="1" max="10"
                            ...
                            onChange={(e) => setSearchParams({ ...searchParams, adults: parseInt(e.target.value) || 1, infants: 0 })}
                            className="border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 p-2 h-auto font-medium text-base w-full"
                ...
                <div className="pl-2 pr-1 w-full sm:w-auto">
                     <Button type="button" onClick={handleSearch} disabled={loading} className="w-full sm:w-auto h-12 bg-yellow-400 hover:bg-yellow-500 text-black font-bold rounded-full flex items-center justify-center gap-2 text-base">
      ...
      {/* Main Page Content - Search Results & Other Sections */}
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
            ...
            {showResults && (
                <section id="search-results" className="py-16">
                    <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Available Ferries</h2>
                    {searchResults.length === 0 ? (
                        <div className="bg-white rounded-xl shadow-md p-12 text-center text-gray-500">
                           <Search className="h-16 w-16 text-gray-400 mx-auto mb-4" />
                           <h3 className="text-xl font-semibold text-gray-800">No results found</h3>
                           <p>Try adjusting your search criteria or choose a different date.</p>
                        </div>
                    ) : (
            ...
            <section style={customFontStyle} className="text-center mt-16 sm:mt-24 md:mt-32">
                <p className="text-gray-500 font-semibold tracking-widest text-xs sm:text-sm">WHAT WE SERVE</p>
                <h2 style={customFontStyle2} className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-800 mt-2">What Will You Get?</h2>
                <p className="text-gray-500 mt-4 max-w-lg mx-auto text-sm sm:text-base px-4">In every single trip that you go, you will get serve like a king.</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12 mt-8 sm:mt-12 px-4">
                    <FeatureCard icon={<MapPin className="h-6 w-6 text-[#6355B5]" />} title="Lot of Travel Places" description="We will provide a lot of places in every single trip that you pick."/>
                    <FeatureCard icon={<Ticket className="h-6 w-6 text-[#6355B5]" />} title="Cheap Travel Packet" description="Every packet travel that you chooses, will not till you." />
                    <FeatureCard icon={<Gem className="h-6 w-6 text-[#6355B5]" />} title="And More Bonus" description="We will make sure that you happy in our trip, so you will get some bonus in your trip."/>
                </div>
            </section>
